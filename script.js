...
function renderFilters() {
    ...
    const t = I18N_DATA[curLang];
    let html = FILTERS_DATA.map(f => {
        const name = curLang === 'cn' ? f.name : f.en;
        const isActive = selectedStyle === f.name ? 'active' : '';
        const promptEscaped = f.prompt.replace(/'/g, "\\'");
        return `<div class="style-card ${isActive}" onclick="selectStyle(this, '${f.name}', '${promptEscaped}')">
...
magicBtn.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(ENDPOINTS.PROMPT, { method: 'POST', body: formData });
...
photoFile.addEventListener('change', (e) => {
    ...
    reader.onload = (ev) => {
        ...
        imgDisplay.classList.remove('pixelating');
...
// 点击上传区域
document.getElementById('emptyState').addEventListener('click', () => photoFile.click());
// 重置画布
...
commandForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const res = await fetch(ENDPOINTS.REPAIR, { method: 'POST', body: formData });
        ...
        // 429 触发广告限流
        if (res.status === 429) {
            if (confirm(I18N_DATA[curLang].alertLimit)) {
                localStorage.setItem('hasHitLimit', 'true');
                adWrapper.style.display = 'flex';
                adToggle.click(); // 触发 toggle change 事件
                // ⚡️ 自动重试
...
async function pollTask(taskId) {
    for (let i = 0; i < 100; i++) {
        const res = await fetch(`${ENDPOINTS.STATUS}?taskId=${taskId}`);
        const data = await res.json();
...
window.onTurnstileRender = function() {
    turnstile.render('#turnstile-container', {
        ...
        callback: (t) => {
            ...
            setTimeout(() => { document.getElementById('verifyModal').style.display = 'none' }, 300);
...
async function fetchAndRenderGallery() {
    const res = await fetch(ENDPOINTS.GALLERY).catch(() => null);
    const data = res ? await res.json() : null;
    if (data && Array.isArray(data) && data.length > 0) {
        allGalleryItems = data;
    } else {
        // Fallback to Filters Data
        allGalleryItems = FILTERS_DATA.map(i => ({ ...i, category: i.cat || '精选', img_url: i.img, title: i.name }));
    }
    renderGalleryTabs();
    renderGalleryGrid();
}
...
function renderGalleryTabs() {
    ...
    galleryTabs.innerHTML = categories.map(cat => 
        `<button class="tab-btn ${cat === currentCategory ? 'active' : ''}" onclick="switchCategory('${cat}')">${cat}</button>`
    ).join('');
}
...
function renderGalleryGrid() {
    ...
    // PRD 2.3 容错设计：如果图片挂了，降级为 Text Mode
    galleryGrid.innerHTML = items.map(i => {
        const promptSafe = i.prompt.replace(/'/g, "\\'");
        const imgUrl = i.img_url || i.img;
        return `
        <div class="gallery-item" data-title="${i.title || i.name}" onclick="applyGallery('${promptSafe}','${imgUrl}')">
            <img src="${imgUrl}" class="gallery-img" onerror="this.style.display='none'; this.parentElement.classList.add('text-mode')">
...
window.openPreview = function() {
    const src = document.getElementById('displayImage').src;
    if (!src) return;
    previewImg.src = src;
...
document.getElementById('shareBtnNav').onclick = openShareModal;
...
async function openShareModal() {
    ...
    canvasContainer.innerHTML = '生成海报中...';
    ...
    try {
        ...
        ctx.fillStyle = '#1d1d1f'; ctx.font = 'bold 72px "Songti SC", "Noto Serif SC", serif'; ctx.textAlign = 'left';
        ...
        if (qrImg) {
            ...
            ctx.drawImage(qrImg, w - 250, h - 300, 170, 170);
        }
        ...
        canvasContainer.appendChild(finalImg);
    } catch (e) {
        console.error(e);
        canvasContainer.innerHTML = '生成失败';
    }
}
...
function initOnboarding() {
    if (!localStorage.getItem('hasSeenOb_v1002')) {
        document.getElementById('obBackdrop').style.display = 'block';
        setTimeout(() => document.getElementById('obBackdrop').classList.add('visible'), 10);
        showStep(0);
    }
}
...
function showStep(idx) {
    ...
    if (idx >= obSteps.length) {
        document.getElementById('obBackdrop').classList.remove('visible');
        setTimeout(() => document.getElementById('obBackdrop').style.display = 'none', 300);
        localStorage.setItem('hasSeenOb_v1002', 'true');
        return;
    }

    curStepIdx = idx;
    const step = obSteps[idx];
    const el = document.getElementById(step.id) || document.querySelector('.magic-bar-wrapper');
    const content = I18N_DATA[curLang].ob[step.key];
    
    el.classList.add('ob-highlight');
    const rect = el.getBoundingClientRect();
    
    tooltip.style.display = 'block';
    document.getElementById('obTitle').innerText = content.t;
    document.getElementById('obDesc').innerText = content.d;
    
    const btnText = idx === obSteps.length - 1 ? I18N_DATA[curLang].obBtnStart : I18N_DATA[curLang].obBtnNext;
    ...
    if (left + tooltipWidth > window.innerWidth - padding) {
        ...
        arrowPos = `${targetCenter - left}px`;
    }
    if (left < padding) {
        left = padding;
        const targetCenter = rect.left + rect.width / 2;
        arrowPos = `${targetCenter - padding}px`;
    }

    document.documentElement.style.setProperty('--arrow-pos', arrowPos);
